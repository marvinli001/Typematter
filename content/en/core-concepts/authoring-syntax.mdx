...
Start new pages with the CLI so required frontmatter is present before you write prose:

```bash
npm run typematter -- new --all-languages --slug guides/install --title.en "Install" --title.cn "安装" --section.en "Guides" --section.cn "指南"
```

Then replace the placeholder body, add the page to `nav.config.ts` if it should be pinned, and run `npm run validate:docs`.
...
~~~mdx
:::endpoint{method="GET" path="/v1/docs/[slug]" title="Fetch a doc page" summary="Returns route metadata." auth="optional" since="0.4.0-rc"}
