...
status: experimental
version: 0.4.0-rc
description: API reference for Typematter CLI, site config, plugins, hooks, and build contracts.
aliases:
  - api reference
  - typematter reference
---
...
Execution order is fixed in build/export: `buildStart -> contentCollected -> pageParsed(*) -> registryReady -> buildEnd`.
...
- `headingDepth`: max heading depth and skip-level detection (e.g. h2 -> h4).
- `missingTranslations`: default-language baseline checks for missing pages, missing key frontmatter fields, and language-only pages.
- `frontmatterSchema`: layered, path-based frontmatter schema validation.
